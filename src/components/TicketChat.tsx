...
export default function TicketChat({ ticketRef, onBack }: TicketChatProps) {
  ...
  // Fetch Chat Messages
  const fetchMessages = useCallback(async (isMounted = true) => {
    try {
  ...
  const initialLetter = (name?: string) => (name ? name.charAt(0).toUpperCase() : 'A');
    ...
    <div className="w-full max-w-[680px] bg-[#0c1017] border border-[#1f2937] rounded-2xl overflow-hidden shadow-2xl flex flex-col h-[650px] text-[#e5e7eb] font-sans my-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-xs text-white">{ticket?.name || 'Submitter'}</span>
              <span className="text-[10.5px] text-[#6b7280]">#{ticketRef}</span>
            </div>
            <p className="text-[10.5px] text-[#9ca3af]">You · {ticket?.email || 'email@example.com'}</p>
          </div>
        </div>
          ...
          <div className="hidden sm:flex items-center gap-1.5 bg-[#161f2e] border border-[#2d3a4e] p-1 rounded-lg text-[10px]">
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded font-bold cursor-pointer transition ${
                senderRole === 'user' ? 'bg-[#2563eb] text-white' : 'text-[#9ca3af]'
              }`}
            >
              User
            </button>
            <button
              ...
              className={`px-2 py-0.5 rounded font-bold cursor-pointer transition ${
                senderRole === 'agent' ? 'bg-[#f59e0b] text-[#3d2a06]' : 'text-[#9ca3af]'
              }`}
            >
              Agent
            </button>
      ...
      <div className="relative flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        <div className="w-full z-10 bg-[#0c1017] border-b border-[#1f2937] flex-shrink-0">
          ...
          >
            <span>
              Ticket details <span className="mx-1">•</span> <strong className="text-white font-semibold">{ticket?.subject || 'Loading...'}</strong>
            </span>
        ...
        {detailsOpen && (
          <div className="absolute top-[41px] left-0 right-0 z-20 px-5 py-4 space-y-4 text-left border-b border-[#1f2937] bg-[#0c1017]/90 backdrop-blur-md shadow-2xl max-h-[260px] overflow-y-auto custom-chat-scrollbar transition-all duration-200">
            <div>
              <div className="text-[10px] font-bold tracking-wider text-[#6b7280] uppercase mb-1">TITLE</div>
              <h4 className="text-[14px] font-bold text-white">{ticket?.subject || 'No subject'}</h4>
            </div>

            <div>
              <div className="text-[10px] font-bold tracking-wider text-[#6b7280] uppercase mb-1">DESCRIPTION</div>
        ...
        {/* Messages Feed */}
        <div ref={feedRef} className="flex-1 p-4 overflow-y-auto space-y-4 bg-[#080b10] flex flex-col custom-chat-scrollbar">
          ...
          {loading ? (
            <div className="text-center text-xs text-[#6b7280] py-8">Loading conversation...</div>
          ) : (
            messages.map((m) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5 text-[10px] text-[#6b7280] mb-0.5 px-1">
                    <span className="font-semibold text-[#9ca3af]">{m.sender_name}</span>
                    <span
